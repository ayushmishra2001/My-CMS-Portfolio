...
- *2026-07-18*: Fixed card peeking in the Projects section carousel by removing negative margins (`-mx-4 md:-mx-6`) from the parent and horizontal padding (`px-4 md:px-6`) from the scroll container. This confines the layout container exactly to the page grid width, allowing the browser to cleanly clip off-screen cards at the grid edge.
- *2026-07-18*: Resolved Next.js static prerendering (SSG) build failure for background customizer page `/admin/settings/background` by loading the WebGL `Warp` component dynamically with `{ ssr: false }`. This stops browser-only canvas WebGL execution from crashing the Node.js compiler.
- *2026-07-18*: Redesigned the Projects section carousel layout and pagination: (1) Changed card widths dynamically relative to parent clientWidth (100% on mobile, 50% on tablet, 33.3% on desktop) to fit exactly 3 cards per frame on desktop without edge slicing. (2) Aligned snap behavior to `snap-start` flush with the left boundary. (3) Configured manual arrow triggers and the auto-scroll interval to scroll page-by-page (by `clientWidth + 24px gap`) at a time, preserving auto-sliding smoothly.
- *2026-07-18*: Resolved the theme toggle bug on the public portfolio page and decoupled it from the CMS panel: (1) Removed the global `ThemeProvider` from `src/app/layout.tsx`. (2) Created two separate context wrappers (`PortfolioThemeWrapper` and `AdminThemeWrapper`) toggling local `dark`/`light` classes on local layout wrapper `div`s. (3) Configured `PortfolioNav` and `AdminHeader` to toggle themes independently using the local hooks, preventing global theme conflicts.
- *2026-07-18*: Implemented performance optimizations for the new dynamic shader background (`Warp` component) to resolve UI lag: (1) Memoized the `Warp` canvas component using React `memo` to avoid re-renders on layout updates, and (2) scaled the background canvas container to `50%` size (`50vw` width and `50lvh` height) and upscaled it back using CSS `scale(2)` and `origin-top-left` to reduce WebGL fragment calculations by 4x.
- *2026-07-15*: Completed the visual redesign of the entire public portfolio to follow the `industrial-brutalist-ui` rules: (1) Replaced standard colors in `globals.css` with unbleached documentation paper substrate (`#F4F4F0`) in light mode, deactivated CRT monitor background (`#0A0A0A`) with white phosphor text (`#EAEAEA`) in dark mode, and Aviation Hazard Red (`#E61919`) as the primary accent color. (2) Enforced strict `--radius: 0px` globally to guarantee 90-degree square corners on all components. (3) Added horizontal CRT scanline patterns in dark mode. (4) Updated custom buttons and header navigation to remove rounded corners and soft shadows.
